...
void max_heapfy(Heap* heap)
{
    ...
    int val_parent = heap->arr[0]; // valor da raiz
    int val_left = 0;
    int val_right = 0;
    ...
    while(!turn) {
        ...
        if(left_idx >= 0 & left_idx <= tail)
        {
            ...
            max_node_val = heap->arr[max_node_idx];

            if(max_node_val > val_parent)
            {
                heap->arr[parent_idx] = max_node_val;
                heap->arr[max_node_idx] = val_parent;
            }

            parent_idx = max_node_idx;
            val_parent = heap->arr[max_node_idx];
        }
        else
        {
            printf("heapfy_response: ok\n");
            turn = true;
        }
    }
}
